<template>
    <div class="play_box"
        ...
        <audio id="audio"
               ...
               @ended = "stop"
               @canplay = "canplay"
               src="http://jiudaifu-sandbox.oss-cn-hangzhou.aliyuncs.com/fxmall/data/courses/bYTjZw4rYx.mp3">
            您的浏览器不支持 audio 标签。
        </audio>
        <div class="music_box">
            <div class="play_bg" :class="{on:isPlay}">
                  <div class="play_btn1" :class="{on:isPlay}" @click="play"></div>
            </div>
            <div class="play_btn2" :class="{on:isPlay}" @click="play"></div>
            <div class="play_info_box">
                <!-- <label class="play_name">{{name}}</label> -->
               <span>{{currentTime | formatTime 'notHasHour'}}</span>
               <!-- </label> --> 
                ...
                <span>{{duration | formatTime 'notHasHour'}}</span>
            </div>
        </div>
    </div>
</template>
...
<style>
    ...
    .play_bg{
        ...
        margin:0 auto;
        position: relative;
        background:url(http://pic30.photophoto.cn/20140114/0008020971744963_b.jpg) no-repeat;
        border-radius: 50%;
    }
    .play_btn1{
        width: 50px;
